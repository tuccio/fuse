...
namespace fuse
{
	...
	inline T & get(matrix<T, N, M> & x)
	{
		return x.m[I];
	}

	template <int I, typename T, int N, int M>
	inline const T & get(const matrix<T, N, M> & x)
	{
		return x.m[I];
	}
	...
	inline void fill(matrix<T, N, M> & m, T d, T f = 0)
	{
		detail::matrix_fill_impl<matrix<T, N, M>, N, M>::fill(m, d, f);
	}

	template <typename T, int N, int M>
	inline matrix<T, N, M> scale(const matrix<T, N, M> & a, const matrix<T, N, M> & b)
	{
		return detail::matrix_scale_impl<T, N, M>::scale(a, b);
	}

	template <typename T, int N, int M>
		inline decltype(auto) transpose(const matrix<T, N, M> & a) 
	{
