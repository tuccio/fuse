...
namespace fuse
{
	...
	namespace detail
	{
		...
		struct newton_raphson_simd
		{
			...
			static inline vec128 FUSE_VECTOR_CALL square_root_ps(vec128 x)
			{
				return _mm_mul_ps(x, reciprocal_square_root_ps(x));
			}

			static inline vec128 FUSE_VECTOR_CALL square_root_ss(vec128 x)
			{
				return _mm_mul_ss(x, reciprocal_square_root_ss(x));
			}

			static inline vec128 FUSE_VECTOR_CALL reciprocal_ps(vec128 x)
			{
				return reciprocal_ps_step(x, _mm_rcp_ps(x));
			}

			static inline vec128 FUSE_VECTOR_CALL reciprocal_ss(vec128 x)
			{
				return reciprocal_ss_step(x, _mm_rcp_ss(x));
			}

			static inline vec128 FUSE_VECTOR_CALL reciprocal_square_root_ps(vec128 x)
			{
				return reciprocal_square_root_ps_step(x, _mm_rsqrt_ps(x));
			}

			static inline vec128 FUSE_VECTOR_CALL reciprocal_square_root_ss(vec128 x)
			{
		...
		private:
			...
			static inline vec128 FUSE_VECTOR_CALL reciprocal_ps_step(vec128 x, vec128 yi)
			{
			...
			static inline vec128 FUSE_VECTOR_CALL reciprocal_ss_step(vec128 x, vec128 yi)
			{
			...
			static inline vec128 FUSE_VECTOR_CALL reciprocal_square_root_ps_step(vec128 x, vec128 yi)
			{
				vec128 t0 = _mm_mul_ps(_mm_mul_ps(x, yi), yi);
			...
			static inline vec128 FUSE_VECTOR_CALL reciprocal_square_root_ss_step(vec128 x, vec128 yi)
			{
				vec128 t0 = _mm_mul_ss(_mm_mul_ss(x, yi), yi);
