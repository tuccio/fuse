...
namespace fuse
{
	...
	public:
		...
		std::shared_ptr<resource> create(const char * name,
		                                 const resource::parameters_type & parameters = resource::parameters_type(),
		                                 resource_loader * loader = nullptr);

		std::shared_ptr<resource> find_by_name(const char * name) const;
		std::shared_ptr<resource> find_by_id(resource::id_type id) const;

		const char * get_type(void) const { return m_type.c_str(); }

		void * get_userdata(void) const { return m_userdata; }

	protected:

		virtual resource * create_impl(const char * name, resource_loader * loader) = 0;
	...
	private:
		...
		std::unordered_map<std::string, resource::id_type>               m_namedResources;
		std::unordered_map<resource::id_type, std::shared_ptr<resource>> m_resources;

		/* Non interlocked implementations */

		std::shared_ptr<resource> find_by_name_unsafe(const char * name) const;
